(function(window){
	...
	var Dictate = function(cfg) {
		...
		this.startListening = function() {
			...
			try {
				console.log('bước 1: bat dau nghe');
				ws = createWebSocket();
			} catch (e) {
		...
		function createWebSocket() {
			...
			ws.onmessage = function(e) {
				...
				console.log('test');
				// res.data = "cong hoa xa hoi";
				// console.log(res);
				if (res.status == 0) {
                    if (res.final){
                    	console.log('test2');
					    config.onResults(res.data, res.worker_name);
                    }
                    else{
                    	console.log('test3');
					    config.onPartialResults(res.data, res.worker_name);
                    }

				} else {
					config.onError(ERR_SERVER, 'Server error: ' + res.status + ': ' + getDescription(res.status));
					console.log(config.onError(ERR_SERVER, 'Server error: ' + res.status + ': ' + getDescription(res.status)));
				}
			}
