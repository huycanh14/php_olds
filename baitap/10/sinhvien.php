...
<?php  
// $totalRecord = $query[khoa.ten_khoa];
// $result = $query->fetch_all(MYSQLI_ASSOC);
$limit = 7;
$currentPage = 1;
if (isset($_GET['page'])){
...
// 
$sql = "SELECT COUNT(*)FROM sinhvien WHERE 1=1";
$query = $db->query($sql);
$totalRecord = $query->fetch_row();
$totalPage = ceil($totalRecord[0]/$limit);
...
<div class="pagination">
	...
	// nếu currentPage > 1 và totalPage > 1 mới hiển thị nút prev
	if ($currentPage > 1 && $totalPage > 1){
	    echo '<a href="sinhvien.php?page='.($currentPage-1).'">Prev</a> | ';
	}
	...
	for ($i = 1; $i <= $totalPage; $i++){
	    ...
	    if ($i == $currentPage){
	        ...
	        echo '<a href="sinhvien.php?page='.$i.'">'.$i.'</a> | ';
	    }
	}

	// nếu currentPage < $totalPage và totalPage > 1 mới hiển thị nút prev
	if ($currentPage < $totalPage && $totalPage > 1){
	    echo '<a href="sinhvien.php?page='.($currentPage+1).'">Next</a> | ';
	}
	?>
</div>