...
if (isset($_POST['submit'])){
	...
	if (count($errors) == 0){
		...
		$sql ="SELECT * FROM post_categories WHERE name = '{$name}' LIMIT 1";
		$query = $db->query($sql);
		$result = $query->fetch_assoc();
		if (!is_null($result)){
			$errors[] = 'Tên Post Category đã tồn tại';
		} else{
			$sql ="SELECT * FROM post_categories WHERE slug = '{$slug}' LIMIT 1";
			$query = $db->query($sql);
			$result = $query->fetch_assoc();
			if (!is_null($result)){
				$errors[] = 'Slug Post Category đã tồn tại';
			} else {
				$sql = "INSERT INTO post_categories (name, slug, image, description, content, created_at, updated_at, meta_title, meta_keyword, meta_description) VALUES ('{$name}', '{$slug}', '{$image}', '{$description}', '{$content}', '{$created_at}', '{$updated_at}', '{$meta_title}', '{$meta_keyword}', '{$meta_description}') ";
				$query = $db->query($sql);
				if ($query){
...
<html>
<head>
	<title>Post Category Add</title>
</head>
<body>
	<h3>Post Category Add</h3>
	<div class="message">
		...
		<table border="1" cellpadding="10">
			...
			<tr>
				<td colspan="2" align="center" style="padding-top: 10px;">
					<input type="submit" name="submit" value="Post Category Add">
				</td>
			</tr>
		</table>
	</form>
