...
if (isset($_POST['submit'])){
	...
	if (count($errors) == 0){
		...
		if (!is_null($result)){
			$errors[] = 'Tên Page đã tồn tại';
		} else{
			$sql ="SELECT * FROM pages WHERE slug = '{$slug}' LIMIT 1";
			$query = $db->query($sql);
			$result = $query->fetch_assoc();
			if (!is_null($result)){
				$errors[] = 'Slug Page đã tồn tại';
			} else {
				$sql = "INSERT INTO pages (name, slug, image, description, content, created_at, updated_at, meta_title, meta_keyword, meta_description) VALUES ('{$name}', '{$slug}', '{$image}', '{$description}', '{$content}', '{$created_at}', '{$updated_at}', '{$meta_title}', '{$meta_keyword}', '{$meta_description}') ";
				$query = $db->query($sql);
				if ($query){
		...
		<table border="1" cellpadding="10">
			...
			<tr>
				<td colspan="2" align="center" style="padding-top: 10px;">
					<input type="submit" name="submit" value="Page Add">
				</td>
			</tr>
		</table>
	</form>
