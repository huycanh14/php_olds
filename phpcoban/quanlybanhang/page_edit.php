...
if (isset($_POST['submit'])){
	...
	if (count($errors) == 0){
		...
		$sql = "SELECT * FROM pages WHERE id = '{$id}' AND id <> '{$page['id']}' LIMIT 1";
		$query = $db->query($sql);
		$result = $query->fetch_assoc();
		if (!is_null($result)){
			$errors[] = 'Id trùng';
		} else {
			$sql = "UPDATE pages SET name = '{$name}', slug = '{$slug}', image = '{$image}', description = '{$description}', content = '{$content}', created_at = '{$created_at}', updated_at = '{$updated_at}', meta_title = '{$meta_title}', meta_keyword = '{$meta_keyword}', meta_description = '{$meta_description}') WHERE id = '{$id}'";
			$query = $db->query($sql);
			if ($query){
				header('location: pages.php');
			} else{
		...
		<table border="1" cellpadding="10">
			...
			<tr>
				<td colspan="2" align="center" style="padding-top: 10px;">
					<input type="submit" name="submit" value="Page Edit">
				</td>
			</tr>
		</table>
	</form>
